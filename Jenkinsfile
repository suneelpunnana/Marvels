    ...
    stages {
        ...
        stage('Uploading artifacts to Ansible'){
            steps{
                ...
                sh 'sshpass -p ${ansadmin_password} ssh -v -o StrictHostKeyChecking=no ansadmin@172.31.22.13 -C \"cd /opt/playbooks/target; wget -O BMI http://18.224.155.110:8081/nexus/content/repositories/devopstraining/comrades/bmi/BMI/BMI-${BUILD_NUMBER}.war\"'
                }
            }
        }
